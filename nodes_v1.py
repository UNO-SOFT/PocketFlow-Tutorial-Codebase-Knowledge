...
class FetchRepo(Node):
    ...
    def exec(self, prep_res):
        ...
        if len(files_list) == 0:
            raise (ValueError(f"Failed to fetch files from {prep_res}"))
...
3. Take it into consideration that the code is primary purpose is to calculate commision in an insurance company and the code base is a small part of a lot bigger code. 
4. Give feedback about the information needed to make better assessment.
...
- Begin with a high-level motivation explaining what problem this abstraction solves{instruction_lang_note}. Make it very minimal and friendly to beginners.

- If the abstraction is complex, break it down into key concepts. Explain each concept one-by-one in a very beginner-friendly way{instruction_lang_note}.

- Evaluate the code based on the notes contained in *.txt files. The code is the truth, extend the .txt documentation from the code.

- Explain how to use this abstraction to solve the use case{instruction_lang_note}.
...
- Then dive deeper into code for the internal implementation with references to files. 
...
Now, directly provide a verbose Markdown output (DON'T need ```markdown``` tags):
"""
